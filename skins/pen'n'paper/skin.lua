...
function on_skin_load()
	...
	font = gfx.load_font("gooddogp.ttf", 16)
...
function on_background_draw()
	...
	screen.draw_text(font, hs, 47, 25, 58, 56, 73)
	screen.draw_text(font, game.score(), 47, 60, 58, 56, 73)
	...
	screen.draw_text(font, t, 132, 4, 58, 56, 73)

	-- Lines
	if mode == "sprint" then
		screen.draw_text(font, "Lines left: " .. (40 - game.lines()), 154, 20, 58, 56, 73, 1, 0)
	else
		screen.draw_text(font, "Lines: " .. game.lines(), 154, 20, 58, 56, 73, 1, 0)
	end

	-- Piece statistics (left panel)
	screen.draw_text(font, game.stat(0), 36, 75, 58, 56, 73)
	screen.draw_text(font, game.stat(1), 36, 99, 58, 56, 73)
	screen.draw_text(font, game.stat(2), 36, 123, 58, 56, 73)
	screen.draw_text(font, game.stat(3), 36, 147, 58, 56, 73)
	screen.draw_text(font, game.stat(4), 36, 171, 58, 56, 73)
	screen.draw_text(font, game.stat(5), 36, 195, 58, 56, 73)
	screen.draw_text(font, game.stat(6), 36, 219, 58, 56, 73)
...
function on_line_clear(data)
	...
	screen.pop_up(154, 88, name, 1500, font, 58, 56, 73, 1, 0)

	-- Show B2B at $lctmid position
	if data.b2b then
		screen.pop_up(154, 104, "Back-2-Back", 1500, font, 58, 56, 73, 1, 0)
	end

	-- Show combo at $lctbot position
	if data.combo and data.combo > 0 then
		screen.pop_up(154, 120, "combo " .. data.combo .. "x", 1500, font, 58, 56, 73, 1, 0)
