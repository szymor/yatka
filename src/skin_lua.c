...
/* ─────────────────────────────────────────────
 * Helper: push a file's content as a Lua string
 * ───────────────────────────────────────────── */
...
/* res.load_font(path, size) → userdata */
...
/* res.draw_text(font, str, x, y [, r, g, b [, alignx, aligny]]) */
...
/* res.draw_bar(x, y, w, h, value, max, dir, rl,gl,bl,al, rr,gr,br,ar) */
static int y_draw_bar(lua_State *L)
{
	...
	switch (dir)
	{
		case 0: /* L→R */ fill = (SDL_Rect){ 0, 0, filled_w, h };
		                 rest = (SDL_Rect){ filled_w, 0, w - filled_w, h }; break;
		case 1: /* R→L */ fill = (SDL_Rect){ w - filled_w, 0, filled_w, h };
		                 rest = (SDL_Rect){ 0, 0, w - filled_w, h }; break;
		case 2: /* U→D */ fill = (SDL_Rect){ 0, 0, w, val * h / maxv };
		                 rest = (SDL_Rect){ 0, val * h / maxv, w, h - val * h / maxv }; break;
		case 3: /* D→U */ fill = (SDL_Rect){ 0, h - val * h / maxv, w, val * h / maxv };
...
/* res.brick_size() → w, h (from the skin struct via upvalue) */
...
/* res.set_brick_size(w, h_offset) — sets C brick dimensions */
...
/* res.set_holdmode("off"|"exchange"|"preserve") */
...
/* res.set_tetromino_color(id, alpha, r, g, b) */
...
/* res.set_bricksprite(image) — wire a loaded image into the brick sprite system */
...
/* res.set_shadow(offx, offy, r, g, b, a) — create shadow surface */
...
static int y_game_ghost_y(lua_State *L)
{
	...
	figures[0]->y = tfy;
	fprintf(stderr, "DBG ghost: fig_y=%d ghost_y=%d steps=%d collide_at=%d\n",
		tfy, ghost_y, steps, steps > 0 ? ghost_y + 1 : ghost_y);
...
{
	char path[512];
	snprintf(path, sizeof path, "%sskin.lua", skin_path);
	FILE *f = fopen(path, "r");
	if (!f) return false;
	fclose(f);
	return true;
...
void skin_lua_init(struct Skin *skin, const char *skin_path)
{
	...
	lua_pushlightuserdata(L, skin);           /* upvalue for y_draw_brick, y_brick_size */
	...
	/* add draw_brick and brick_size aliased into res table too */
...
/* ─────────────────────────────────────────────
 * Helper: call a Lua function with no args and
 *          no return value (pcall guarded)
 * ───────────────────────────────────────────── */
...
void skin_lua_draw_background(struct Skin *skin) { call_lua_void(skin, "draw_background"); }
